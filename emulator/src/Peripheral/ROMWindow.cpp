...
namespace casioemu
{
	void SetupRegion(MMURegion &region, size_t region_base, size_t size, size_t rom_base, bool strict_memory, Emulator& emulator, std::string description = {})
	{
	...
	void ROMWindow::Initialise()
	{
		...
		case HW_ES_PLUS:
			...
			SetupRegion(regions[0], 0x00000, 0x08000, 0x00000, strict_memory, emulator);
			SetupRegion(regions[1], 0x10000, 0x10000, 0x10000, strict_memory, emulator);
			SetupRegion(regions[2], 0x80000, 0x10000, 0x00000, strict_memory, emulator);
			break;

		case HW_CLASSWIZ:
			regions.reset(new MMURegion[5]);
			SetupRegion(regions[0], 0x00000, 0x0D000, 0x00000, strict_memory, emulator);
			SetupRegion(regions[1], 0x10000, 0x10000, 0x10000, strict_memory, emulator);
			SetupRegion(regions[2], 0x20000, 0x10000, 0x20000, strict_memory, emulator);
			SetupRegion(regions[3], 0x30000, 0x10000, 0x30000, strict_memory, emulator);
			SetupRegion(regions[4], 0x50000, 0x10000, 0x00000, strict_memory, emulator);
			break;
		}
	}
